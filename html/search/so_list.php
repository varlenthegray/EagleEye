              ...
              <div class="col-md-3">
                <table style="width:100%;margin-top:8px;" class="table table-custom-nb">
                  <?php if((bool)$_SESSION['userInfo']['dealer']) {
                    ...
                    $so_contacts_qry = $dbconn->query("SELECT c.*, c2.description FROM contact_associations soc LEFT JOIN contact c ON soc.contact_id = c.id LEFT JOIN contact_types c2 ON c.type = c2.id WHERE so_id = '{$result['id']}' ORDER BY c.first_name, c.last_name ASC");
