...
switch($_REQUEST['action']) {
    case 'update_alerts':
        ...
        $alert_qry = $dbconn->query("SELECT * FROM alerts WHERE alert_user = {$_SESSION['userInfo']['id']} AND time_viewed IS NULL");
...
HEREDOC;
            ...
            echo <<<HEREDOC
<a class="nav-link dropdown-toggle arrow-none waves-light waves-effect" data-toggle="dropdown" href="#" role="button" aria-haspopup="false" aria-expanded="false">
    <i class="zmdi zmdi-notifications-active noti-icon wiggler"></i>
</a>
<div class="dropdown-menu dropdown-menu-right dropdown-arrow dropdown-lg" aria-labelledby="Preview">
    <!-- item-->
    <div class="dropdown-item noti-title">
        <h5><small><span class="label label-danger pull-xs-right" id="notification_count">$count</span>Notification$plural</small></h5>
    </div>
