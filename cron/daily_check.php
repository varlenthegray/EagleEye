...
// checking for delivery date within 3 days of delivery date
$sod_plus_three = $sod + 259200;

$del_qry = $dbconn->query("SELECT * FROM rooms r LEFT JOIN sales_order so ON r.so_parent = so.so_num WHERE delivery_date BETWEEN $sod AND $sod_plus_three;");

if($del_qry->num_rows > 0) {
  while($del = $del_qry->fetch_assoc()) {
    $to = 'cindy@smcm.us';
    $subject = "{$del['so_parent']}{$del['room']}{$del['iteration']}_{$del['product_type']}{$del['days_to_ship']}--{$so['room_name']}";
