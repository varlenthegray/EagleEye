...
class queue {
  function suspendOps($user_id) {
    global $dbconn;

    $op_queue_qry = $dbconn->query("SELECT * FROM op_queue WHERE active = TRUE AND active_employees LIKE '%\"$user_id\"%'");
      ...
      case 'Break':
        ...
        if($admin_qry->num_rows > 0) { // if we were able to get the operation
          ...
          if($break_qry->num_rows === 0) {
            // suspend all existing operations
            ...
            echo displayToast("success", "Successfully started break. All operations paused.", "Started Break");
          } else {
            echo displayToast("info", "You are currently on break, you cannot start another one!", "Unable to Start Break");
          }
        } else {
          dbLogSQLErr($dbconn);
        }
