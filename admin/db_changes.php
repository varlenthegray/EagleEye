...
if($admin_flag_qry->num_rows > 0) {
    ...
    if(!(bool)$admin_flag['value']) {
        echo ($dbconn->query("ALTER TABLE log_cron CHANGE name `desc` TEXT;")) ? "Successful with alter Cron Table.<br />" : "<b>Error</b> with alter Cron Table.<br />";
        echo ($dbconn->query("CREATE TABLE dealers (id INT PRIMARY KEY AUTO_INCREMENT,dealer_id VARCHAR(10),contact VARCHAR(150),dealer_name VARCHAR(150),physical_address VARCHAR(150),physical_city VARCHAR(50),physical_state VARCHAR(2),physical_zip VARCHAR(10),shipping_address VARCHAR(150),shipping_city VARCHAR(50),shipping_state VARCHAR(2),shipping_zip VARCHAR(10),phone VARCHAR(15),email VARCHAR(150),multiplier DOUBLE,ship_zone CHAR,sales_rep_number INT(5));")) ? "Successful with create Dealers table.<br />" : "<b>Error</b> with create Dealers Table.<br />";
        ...
        echo ($dbconn->query("UPDATE admin_flags SET value = TRUE WHERE flag = 'db_updated';")) ? "Successful with changing Door & Drawer to Drawer & Doors for Bracket Completed op.<br />" : "<b>Error</b> with changing Door & Drawer to Drawer & Doors for Bracket Completed op.<br />";
